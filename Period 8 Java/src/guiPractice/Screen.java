...
public abstract class Screen {
	...
	public void remove(Visible v){
		 ...
		 * The 1st integer to be removed is 6, at index 2 since it gets removed,
		 * 7 moves form index 3 to index 2.
		 * Your list is now (4,5,7)
		 * After you increment i, i becomes 3.
		 * This is out of bounds, so 7 never gets removed. Instead when an object
		 * is removed, decrease i to compensate for change in size:
		 * 
		 * CORRECT
		 * for(int i=0; i<list.size(); i++){
		 * 	if(list.get(i)>5){
		 ...
		 * }
		 * 
		 * for this reason the following do not even work!
		 * 
		 * for(Integer i: list){
		 * 	if(i>5)list.remove(i);
		 * }
		 * Does not  work BECAUSE remove changes the size!
		 * 
		...
		/**
		 * This removes the object that has the same identity as v, not an object
		 * that is equal to v
		 */
	...
	public void moveToBack(Visible v){
		if(viewObjects.contains(v)){
			viewObjects.remove(v);
			viewObjects.add(0,v);
			//moves all objects with index>-n forward by 1, increases size by 1
